...
adminRouter.post('/login', loginLimiter, (req, res) => {
    ...
    if (!user) {
        logActivity({ actor: username, action: 'login_failed', ip: req.ip });
        return res.status(401).json({ status: 'error', message: 'Invalid username or password.' });
    }

    recordLogin(username, req.ip);
    logActivity({ actor: username, action: 'login_success', ip: req.ip });
...
adminRouter.post('/logout', requireAdmin, (req, res) => {
    logActivity({ actor: req.admin.username, action: 'logout', ip: req.ip });
...
adminRouter.post('/change-password', requireAdmin, (req, res) => {
    ...
    logActivity({ actor: req.admin.username, action: 'password_changed', ip: req.ip });
    res.json({ status: 'success' });
});
